...
class GUIDSL(val title: String, val rows: Byte) {
     ...
     */
    operator fun set(range: IntRange, block: (Int) -> ItemStack) {
        range.forEach {
     ...
     */
    operator fun set(range: IntRange, itemStack: ItemStack) {
        set(range) { itemStack }
    }
     ...
     */
    @JvmName("setOnClick")
    fun setItems(range: IntRange, block: (Int) -> Pair<ItemStack, OnClickEvent.() -> Unit>) {
        range.forEach {
            val pair = block(it + 1)
            gui.onClick[it] = {
                if (it in gui.onClick) gui.onClick[it]?.invoke(this)
                pair.second(this)
            }
            gui.spigot.setItem(it, pair.first)
        }
    }

    /**
     * This fills and replace the indices of [range] with [item].
     */
    @JvmName("setOnClick")
    operator fun set(range: IntRange, item: Pair<ItemStack, OnClickEvent.() -> Unit>) {
        set(range) {
            gui.onClick[it] = {
                if (it in gui.onClick) gui.onClick[it]?.invoke(this)
                item.second(this)
 ...
 */
fun rowRange(row: Int, from: Int, to: Int): IntRange {
    return ((row - 1) * 9).run {
